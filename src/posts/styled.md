...
The link list shown above was generated by adding the following shortcode to the page:

<code>
&#123;% pageLinks 1, 0, true, true, "page-links" %&#125;
</code>

This example uses the following positional parameters:

+ A list type of 1 (ordered) 
+ Minumum links (0) which causes the shortcode to generate all links
+ Open in new tab (true)
+ External links only (true)
+ List class ("page-links")

With the `listClass` configuration option, the provided class is added to the link list container. In this case, it takes advantage of some additional styling added to the projects `/assets/css/custom.css` file.
